...
/* USER CODE BEGIN 0 */
/*
void Get_New_Sensor_Data(uint8_t* buffer_ptr, uint16_t size)
{
  // This function simulates getting new data and filling the provided buffer part
  for(uint16_t i = 0; i < size; i++) {
    // Replace with actual sensor read or data generation
    buffer_ptr[i] = (uint8_t)(i); // Example: time-varying data
  }
}

static void Init_SPI_Double_Buffer(void)
{
  // Initialize first half
  Get_New_Sensor_Data(&spi_tx_double_buffer[0], SPI_BLOCK_SIZE);
  // Initialize second half
  Get_New_Sensor_Data(&spi_tx_double_buffer[SPI_BLOCK_SIZE], SPI_BLOCK_SIZE);
}
*/
/* USER CODE END 0 */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  // 1. Create PDU
	    ...
	    /*
		for (uint32_t i = 0; i < (upscaled_length >> 2); ++i) {
			ble_tx_buffer[i] = __REV(ble_tx_buffer[i]);
		}
		*/
